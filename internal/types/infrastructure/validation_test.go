...
func TestInstanceRequest_Validate(t *testing.T) {
	tests := []struct {
		...
		{
			name: "valid request with payload",
			request: &InstanceRequest{
				Name:              "valid-instance",
				Provider:          "digitalocean",
				NumberOfInstances: 1,
		...
		{
			name: "valid request with payload but no path",
			request: &InstanceRequest{
				Name:              "valid-instance",
				Provider:          "digitalocean",
				NumberOfInstances: 1,
		...
		{
			name: "invalid payload path - not absolute",
			request: &InstanceRequest{
				Name:              "valid-instance",
				Provider:          "digitalocean",
				NumberOfInstances: 1,
				...
				SSHKeyName:        "test-key",
				Payload:           "test-payload-content",
				PayloadPath:       "relative/path/payload.bin",
			},
			wantErr: true,
			errMsg:  "payload_path must be an absolute path",
		},
		{
			name: "invalid payload path - contains relative components",
			request: &InstanceRequest{
				Name:              "valid-instance",
				Provider:          "digitalocean",
				NumberOfInstances: 1,
				...
				SSHKeyName:        "test-key",
				Payload:           "test-payload-content",
				PayloadPath:       "/tmp/../etc/payload.bin",
			},
			wantErr: true,
			errMsg:  "cannot contain relative path components",
		},
		{
