...
func (s *InstanceRepositoryTestSuite) TestUpdateByName() {
	...
	err := s.instanceRepo.UpdateByName(s.ctx, instance.Name, map[string]interface{}{
	...
	s.NoError(err)
	s.Equal("192.0.2.100", updated.PublicIP)
	s.Equal(instance.Status, updated.Status) // Other fields should remain unchanged
	...
	s.NoError(err)
	s.Equal(models.InstanceStatusReady, updated.Status)
	s.True(updated.IsProvisioned)
	s.Equal("192.0.2.100", updated.PublicIP) // Previous updates should be preserved

	// Test updating non-existent instance
	err = s.instanceRepo.UpdateByName(s.ctx, "non-existent", map[string]interface{}{
		"status": models.InstanceStatusReady,
	})
	s.NoError(err) // GORM doesn't return error for updates that affect 0 rows
}

func (s *InstanceRepositoryTestSuite) TestProvisioningWorkflow() {
	...
	s.False(instance.IsProvisioned) // Should be false by default

	// Simulate instance creation completion
	err := s.instanceRepo.UpdateStatusByName(s.ctx, instance.Name, models.InstanceStatusReady)
	s.NoError(err)
	...
	s.NoError(err)
	s.Equal(models.InstanceStatusReady, updated.Status)
	s.False(updated.IsProvisioned) // Should still be false
	...
	s.NoError(err)
	s.True(updated.IsProvisioned)
	s.Equal(models.InstanceStatusReady, updated.Status) // Status should remain unchanged
}
