...
type TaskListByInstanceParams struct {
	InstanceID uint   `json:"instance_id"` // From path
...
func (p TaskListByInstanceParams) Validate() error {
	...
	// Optional: Validate p.Action against known models.TaskAction values if desired,
	// or let the service/repo layer handle unknown actions (which might return an empty list).
	return nil
}
